...
def get_floor_mask(session, img_bgr, conf_threshold=0.25):
    ...
    proto_output = outputs[1] # (1, 32, 160, 160)

    # Transpose: (1, 37, 8400) → (8400, 37)
    detections = det_output[0].transpose(1, 0)
    ...
    for det in detections:
        ...
        # Class scores ada di index 4 sampai 4+nc
        # nc=1 jadi class score ada di det[4], tidak ada conf terpisah
        cls_score = float(det[4])
        if cls_score < conf_threshold:
            continue

        found = True
        mask_coef = det[5:37]  # 32 koefisien mask

        mask = np.einsum('c,chw->hw', mask_coef, proto)
        mask = 1 / (1 + np.exp(-mask))

        # Koordinat dalam skala imgsz
